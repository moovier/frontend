              ...
              <Row justify="between">
                ...
                <Col xs={12} sm={6} md={8}>
                  <div className="movie-details__content">
                    ...
                    <div>
                      <b>Languages: </b>{" "}
                      {spoken_languages.map((i: any) => i.name + ",")}
                    </div>
                    <br />
                    <br />
                    <Title level={4}>Genres</Title>
                    <div>
                      {this.props.allGenres.length !== 0 && (
                        <GenreTags
                          genres_ids={genres.map((e: any) => e.id)}
            ...
            <Col xs={12} sm={12} md={3}>
              ...
              {production_companies.map((c: any) => (
                <div key={c.id} className="movie-company">
                  <span className="movie-company__name">
              ...
              {production_countries.map((c: any, k: number) => (
                <div key={k} className="movie-country">
                  <span className="movie-country__name">
                    {c.name} ({c.iso_3166_1})
                  </span>
                </div>
              ))}
  ...
  render() {
    ...
    return (
      <React.Fragment>
        {data ? (
          <BackdropWrapper
            image_path={`${API_IMG_URL}${API_IMG_BACKDROP_SIZE}/${data &&
              data.backdrop_path}`}
          >
            <AppHeader
              showButton={true}
              onPopularMovies={() => this.props.history.push("/home")}
            />
